    ...
    <div className="p-4 space-y-5">
      ...
      <div>
        ...
        <div className="space-y-3">
          <div>
            <label className="text-xs font-medium text-gray-700 dark:text-gray-300 block mb-1">Tipo de gráfico</label>
            <div className="grid grid-cols-5 gap-1">
              {CHART_TYPES.map((ct) => (
                <button key={ct.value} onClick={() => setChartType(ct.value)}
                  title={ct.label}
                  className={`flex flex-col items-center gap-0.5 p-1.5 rounded-lg border text-xs transition-all ${
                    chartType === ct.value
          ...
          <div>
            <label className="text-xs font-medium text-gray-700 dark:text-gray-300 block mb-1">Agrupar por *</label>
          ...
          {chartType !== "pie" && chartType !== "doughnut" && (
            <>
              <div>
                <label className="text-xs font-medium text-gray-700 dark:text-gray-300 block mb-1">Coluna de valor</label>
              ...
              {valueCol && (
                <div>
                  <label className="text-xs font-medium text-gray-700 dark:text-gray-300 block mb-1">Agregação</label>
                  <div className="flex flex-wrap gap-1">
                    {AGG_FUNCS.filter(a => a.value !== "count").map((a) => (
          ...
          <div>
            <label className="text-xs font-medium text-gray-700 dark:text-gray-300 block mb-1">Título (opcional)</label>
            <input value={title} onChange={(e) => setTitle(e.target.value)}
              placeholder="Título do gráfico..."
              className="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>

...
export default function App() {
  ...
  useEffect(() => { localStorage.setItem("dashboardCharts", JSON.stringify(charts)); }, [charts]);
  ...
  const addChart = useCallback((config) => {
    setCharts((prev) => [...prev, { id: Date.now().toString(), ...config }]);
  }, []);
  const removeChart = useCallback((id) => setCharts((prev) => prev.filter((c) => c.id !== id)), []);
  const updateChart = useCallback((id, upd) => setCharts((prev) => prev.map((c) => c.id === id ? { ...c, ...upd } : c)), []);
  const applySuggestion = (sug) => addChart({ ...sug, colorIndex: charts.length % CHART_COLORS.length });
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {dataset && <p className="text-xs text-gray-500 dark:text-gray-400">{dataset.filename} — {dataset.rows.toLocaleString()} linhas</p>}
          </div>
        </div>
        <div className="flex items-center gap-3">
          {dataset && (
            <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
              {["charts","table"].map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  className={`px-3 py-1 rounded text-sm font-medium transition-all ${activeTab === tab ? "bg-white dark:bg-gray-600 text-blue-600 dark:text-blue-400 shadow-sm" : "text-gray-600 dark:text-gray-300"}`}>
      ...
      <main className="flex flex-col lg:flex-row h-[calc(100vh-57px)]">
        <aside className="w-full lg:w-72 xl:w-80 bg-white dark:bg-gray-800 border-b lg:border-b-0 lg:border-r border-gray-200 dark:border-gray-700 overflow-y-auto scrollbar-thin flex-shrink-0">
          {!dataset ? (
            <div className="p-4"><FileUpload onUpload={handleUpload} loading={loading} /></div>
          ) : (
            <ControlPanel
              dataset={dataset} onAddChart={addChart} suggestions={suggestions}
              onApplySuggestion={applySuggestion} chartCount={charts.length}
              onNewUpload={() => { setDataset(null); setCharts([]); setSuggestions([]); }} API={API}
            />
          )}
        </aside>
        <div className="flex-1 overflow-auto">
          {!dataset ? (
            ...
            <ChartGrid charts={charts} onRemove={removeChart} onUpdate={updateChart} API={API} colors={CHART_COLORS} />
          ) : (
